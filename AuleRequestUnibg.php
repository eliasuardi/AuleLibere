...
extends AuleRequest
{
    ...
    { 
        ...
        $this->service_endpoint = "http://www03.unibg.it/orari/orario_giornaliero.php?";
    ...
    //////// get word request
    function get_classroom_free_hours( $db, $idfacolta, $date)
    {
        $url = $this->service_endpoint . "db=" . $db . "&idfacolta=" . $idfacolta . "&data=" . urlencode( $date);
    ...
    private function get_lesson_times_from_tr_td( $span)
    {
        ...
        {        
        	for($i=0; $i<$span->length; $i++)
        	{
        		...
        		if($children->length == 4 && strpos(strtolower($children->item(0)->textContent), 'aula'))
        		{
        			$lessons_array[] = strtolower(preg_replace('/\s\s+/', ' ', trim($this->generate_classroom_key($children->item(0)->textContent))));
        			$lessons_array[] = strtolower(preg_replace('/\s\s+/', ' ', trim($children->item(2)->textContent)));

        		}
        		else if($children->length == 1 && strpos(strtolower($children->item(0)->textContent), 'utenza'))
        		{
        			$classroom = $span->item($i+3)->childNodes;
        			$lessons_array[] = strtolower(preg_replace('/\s\s+/', ' ', trim(preg_replace('/inform.|infor./', '( '.trim($children->item(0)->textContent).')', $this->generate_classroom_key($classroom->item(0)->textContent)))));
        			$lessons_array[] = strtolower(preg_replace('/\s\s+/', ' ', trim($classroom->item(2)->textContent)));
        		}
        	}
        }
    ...
    private function parse_lesson_data( $lesson_array)
    {
    	...
    	for($i=0;$i<sizeof($lesson_array);$i=$i+2)
    	{
    		$key = $lesson_array[$i];
    		...
    		{
    			$classroom_array[$key] = $lesson_array[$i+1];
    			continue;
    		}
    		
    		$busy_hour = $lesson_array[$i+1];
